...
export class QuoteBox extends React.Component {
  ...
  async componentDidMount() {
    const quotesListJSON = await fetch(quotesSourceURL);
    try {
      if (!quotesListJSON.ok) {
        const message = `An error has occured: ${quotesListJSON.status}`;
        throw new Error(message);
      }
    } catch(error) {
      alert(error);
    }
    const quotesFromJSON = await quotesListJSON.json();
    this.setState({
